...
by every run it produced, so the same code run on four cases can be found again as one group.
"""
...
def list_runs(case: pathlib.Path) -> list[dict]:
    ...
    for strategy in sorted(p for p in root.iterdir() if p.is_dir()):
        for folder in sorted(p for p in strategy.iterdir() if p.is_dir() and (p / "fused_plots.parquet").exists()):
