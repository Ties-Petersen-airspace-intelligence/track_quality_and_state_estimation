...
const NUMERIC = [
  ...
  { id:"late", name:"lateness, received minus position time", unit:"s", value:i => (R().r[i] - R().t[i]) / 1000, step:0.1 },
...
// a filter block: a header that switches it on and off, a body that is greyed while off
function block(id, name, body) {
  const el = document.createElement("div"); el.className = "filter"; el.dataset.id = id;
  el.innerHTML = `<div class="fhead"><input type="checkbox"><span class="name">${name}</span></div><div class="fbody"></div>`;
  el.querySelector(".fbody").append(...[].concat(body));
  const cb = el.querySelector(".fhead input");
  el.querySelector(".fhead").onclick = e => { if (e.target !== cb) cb.checked = !cb.checked; state[id].on = cb.checked; el.classList.toggle("on", cb.checked); changed(); };
  return el;
}
...
function sourceFilter() {
  ...
  const el = block("source", "source and plot kind", html(body + "</div>"));
...
function textFilter(f) {
  ...
  const el = block(f.id, f.name, html(`<div class="field"><span class="label">keep plots that</span><select><option value="set">have it set</option><option value="unset">do not have it set</option></select></div>`));
...
function outcomeFilter() {
  ...
  const el = block("outcome", "what a strategy did with it", html(`<div class="field"><span class="label">strategy run</span><select class="outcome-run"></select></div><div class="opts outcome-states" style="margin-top:0.35rem"></div><div class="opts outcome-reasons" style="margin-top:0.35rem"></div>`));
...
export function computePass() {
  ...
  $("filterCount").textContent = tests.length ? `${tests.length} on · ${fmtN(failed)} ${hide ? "hidden" : "greyed"}` : "";
  $("dbgFiltered").textContent = fmtN(failed);
}
export const hideFailed = () => document.querySelector('input[name="fail"]:checked').value === "hide";

function changed() { computePass(); bus.draw(); }
