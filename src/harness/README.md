...
- uAvionix `common.ground_speed_kt` is not in knots. The median over the hour is 0.098 while ADS-B Exchange says 395 kt for the same traffic, and the value equals the raw `airborne_ground_vector.ground_speed`, which ASTERIX CAT021 defines in nautical miles per second. Multiply by 3600 to get knots. Fixed in the integration under TS-1929 (September 2026); data pulled before that keeps the raw value. The wrong value reaches production: the 54 uAvionix rows in the final fused table have a median ground speed of 0.03 kt. Any strategy using uAvionix speed must convert, and the source adapter should be fixed.
